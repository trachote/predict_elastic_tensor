from se3t.utils.utils_profiling import *  # load before other local modules
...
from se3t.equivariant_attention.from_se3cnn import utils_steerable
from se3t.equivariant_attention.fibers import Fiber, fiber2head
from se3t.utils.utils_logging import log_gradient_norm
...
class GConvSE3(nn.Module):
    ...
    """
    def __init__(self, f_in, f_out, self_interaction: bool=False, edge_dim: int=0, flavor='skip', mid_dim: int=32):
        ...
        for (mi, di) in self.f_in.structure:
            for (mo, do) in self.f_out.structure:
                self.kernel_unary[f'({di},{do})'] = PairwiseConv(di, mi, do, mo, edge_dim=edge_dim, mid_dim=mid_dim)
...
class RadialFunc(nn.Module):
    ...
    def __init__(self, num_freq, in_dim, out_dim, edge_dim: int=0, mid_dim: int=32):
        """NN parameterized radial profile function.
...
class PairwiseConv(nn.Module):
    ...
    def __init__(self, degree_in: int, nc_in: int, degree_out: int,
                 nc_out: int, edge_dim: int=0, mid_dim: int=32):
        ...
        self.rp = RadialFunc(self.num_freq, nc_in, nc_out, self.edge_dim, self.mid_dim)
...
class GConvSE3Partial(nn.Module):
    ...
    def __init__(self, f_in, f_out, edge_dim: int=0, x_ij=None, mid_dim: int=32):
...
class GSE3Res(nn.Module):
    ...
    def __init__(self, f_in: Fiber, f_out: Fiber, edge_dim: int=0, div: float=4,
        ...
        # Projections
        self.GMAB['v'] = GConvSE3Partial(f_in, self.f_mid_out, edge_dim=edge_dim, x_ij=x_ij, mid_dim=mid_dim)
        self.GMAB['k'] = GConvSE3Partial(f_in, self.f_mid_in, edge_dim=edge_dim, x_ij=x_ij, mid_dim=mid_dim)
